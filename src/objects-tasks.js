...
function mergeObjects(objects) {
  const newObj = objects.reduce((acc, obj) => {
    Object.entries(obj).forEach((element) => {
      ...
      } else acc[element[0]] = 0 + element[1];
    });
    return acc;
  }, {});
  return newObj;
...
function makeWord(lettersObject) {
  const resArray = [];
  const keysArray = Object.keys(lettersObject);
  keysArray.forEach((arr, index) => {
    const valuesSubArray = lettersObject[arr];
    valuesSubArray.forEach((element) => {
      resArray[element] = keysArray[index];
    });
  });
...
function sellTickets(queue) {
  const changeArr = [];
  const cost = 25;
  function giveChange(banknote) {
    if (banknote === 25) {
      changeArr.push(banknote);
      return true;
    }
    let changeToGive = banknote - cost;
    if (changeToGive === 0) return true;
    for (let i = 0; i < changeArr.length; i += 1) {
      changeToGive -= changeArr[i];
      if (changeToGive === 0) return true;
    }
    return false;
  }
  for (let i = 0; i < queue.length; i += 1) {
    if (!giveChange(queue[i])) return false;
  }
  return true;
}
